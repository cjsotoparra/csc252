...
#	print "    var    : $line\n";
	...
	if ($line =~ m/^(addu?|subu?|slt|and|or|nor|xor|mul) \$[a-z0-9]+ ?, ?\$[a-z0-9]+ ?, ?\$[a-z0-9]+$/)
	{
		next;
	}
	...
	if ($line =~ m/lui \$[a-z0-9]+ ?, ?(-?[0-9]{1,5}|0x[0-9a-fA-F]{1,4})/)
	{
		next;
	}
	...
	# along with jr
	if ($line =~ m/(mult|div) \$[a-z0-9]+ ?, ?\$[a-z0-9]+/)
	{
		next;
	}
